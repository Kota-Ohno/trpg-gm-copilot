...
- `npx pnpm@11.0.8 run check` passed after API-key persistence hardening:
  ...
  - 131 tests passed in the latest full check after adding landmark accessibility and provider error redaction regression coverage.
...
- Provider error messages now redact `Bearer ...` and `sk-...` patterns before user-visible display, with extraction and transcription regression tests covering upstream secret echoing.
