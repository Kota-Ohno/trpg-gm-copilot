...
| Color accessibility. | Contrast spot check recorded in terminal: white on `public-seaglass` moved from 3.34:1 to 4.87:1; `npx pnpm@11.0.8 run check` passed. | Main token pair now reaches AA for normal text. Full automated per-component contrast coverage is not present. | Partially covered |
| API-key risk minimization. | `src/App.tsx`, `src/components/provider-settings-card.tsx`, `src/lib/campaign.ts`, `src/lib/diagnostics.ts`, tests. | Provider keys are session-only; legacy storage keys are removed; unknown secret-like JSON fields are stripped from normalization and exports; diagnostics exclude secrets. | Covered |
| Tests and build quality. | Latest `npx pnpm@11.0.8 run check`: 14 test files, 127 tests, production build. | Focused regression tests cover provider copy, diagnostics, export sanitization, and unknown secret-field stripping. | Covered |
...
- Full component-level automated contrast/accessibility coverage is not implemented. Current evidence is a targeted token contrast calculation plus rendered smoke screenshots.
