...
| Imported JSON must not smuggle secret fields forward. | `src/lib/campaign.ts` rebuilds normalized/exported library, campaign, session, live-log, extraction item, and run objects from allowlisted fields only; tests cover unknown `apiKey`/`providerSecrets` stripping. |
